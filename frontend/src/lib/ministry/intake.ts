...
export class MinistryIntakeError extends Error {
  constructor(public readonly code: "INVALID_ISSUE_ID" | "ISSUE_NOT_FOUND" | "ISSUE_NOT_ACCEPTED") {
    super("The city issue could not be accepted by the demonstration Ministry intake.");
    this.name = "MinistryIntakeError";
  }
}

function priorityForIssue(category: CityIssueCategory, confidence: number): MinistryIssuePriority {
  if ((category === "WATER_LEAKAGE" || category === "WATERLOGGING") && confidence >= 0.9) return "HIGH";
  if (
...
function getAcceptedCityIssue(cityIssueId: string): CityMonitorIssue {
  if (typeof cityIssueId !== "string" || !/^(kartaview|demo):[^\s:]+$/.test(cityIssueId) || cityIssueId.length > 120) {
  ...
  const issue = store.getIssue(cityIssueId, expectedProvider) ?? store.getIssue(sourceImageId, expectedProvider);
  if (!issue) throw new MinistryIntakeError("ISSUE_NOT_FOUND");
  if (issue.provider !== expectedProvider) throw new MinistryIntakeError("ISSUE_NOT_FOUND");
  const processing = store.getProcessing(issue.sourceImageId, issue.provider);
  if (!processing || processing.status !== "accepted_issue") throw new MinistryIntakeError("ISSUE_NOT_ACCEPTED");
  return issue;
}

function createRecord(issue: CityMonitorIssue): MinistryIssueRecord {
  const store = getMinistryIssueStore();
  const source = issue.provider === "demonstration" ? "DEMONSTRATION_SIMULATION" : MINISTRY_SOURCE;
  return store.create({
    source,
